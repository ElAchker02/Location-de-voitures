...
namespace Location_des_Voitures.Forms
{
    public partial class FormMarques : Form
    {
        ...
        private void Charger_db()
        {
            ...
            GLB.Connection.Open();
            GLB.Cmd = new SqlCommand("select * from Marques", GLB.Connection);
            GLB.dr = GLB.Cmd.ExecuteReader();
            while (GLB.dr.Read())
            {
                dgvMarque.Rows.Add(GLB.dr[0], GLB.dr[1]);
            }
            GLB.dr.Close();
            GLB.Connection.Close();
        }
        ...
        {
            txtMarque.Text = dgvMarque.CurrentRow.Cells[1].Value.ToString();
            position = dgvMarque.CurrentRow.Index;
        }
        private void Select_dgv(int i)
        {
            txtMarque.Text = dgvMarque.Rows[i].Cells[1].Value.ToString() ;
        ...
        {
            if (dgvMarque.Rows.Count > 0)
            {
                dgvMarque.Rows[0].Selected = true;
                position = 0;
            }
            Select_dgv(position);
        ...
        private void btnPrecedent_Click(object sender, EventArgs e)
        {
            if (dgvMarque.Rows.Count > 0)
            {
                ...
                if (pos == -1)
                    return;
                dgvMarque.ClearSelection();

                dgvMarque.Rows[pos].Selected = true;
                position = pos;
            }
            Select_dgv(position);
        }
        ...
        private void btnDernier_Click(object sender, EventArgs e)
        {
            if (dgvMarque.Rows.Count > 0)
            {
                int row = dgvMarque.Rows.Count - 1;
                dgvMarque.Rows[row].Selected = true;
                position = row;
            }
            Select_dgv(position);
            ...
            try
            {
                if (txtMarque.Text!="")
                {
                    if (GLB.Connection.State == ConnectionState.Open)
                        GLB.Connection.Close();
                    GLB.Connection.Open();
                    GLB.Cmd = new SqlCommand($"insert into Marques values ('{txtMarque.Text.Trim()}')", GLB.Connection);
                    GLB.Cmd.ExecuteNonQuery();
                    MessageBox.Show("La Marque a été bien ajouté", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    GLB.Connection.Close();
                    dgvMarque.Rows.Clear();
                    Charger_db();
                }
                else
                {
            ...
            try
            {
                if (txtMarque.Text != "")
                {
                    if (GLB.Connection.State == ConnectionState.Open)
                        GLB.Connection.Close();
                    GLB.Connection.Open();
                    GLB.Cmd = new SqlCommand($"update Marques set Marque = '{txtMarque.Text.Trim()}' where id_Marque ={dgvMarque.Rows[position].Cells[0].Value} ", GLB.Connection);
                    GLB.Cmd.ExecuteNonQuery();
                    MessageBox.Show("La Marque a été bien modifié", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    GLB.Connection.Close();
                    dgvMarque.Rows.Clear();
                    Charger_db();
                }
        ...
        private void btnSupprimer_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    if (GLB.Connection.State == ConnectionState.Open)
                        GLB.Connection.Close();
                    GLB.Connection.Open();
                GLB.Cmd = new SqlCommand($"delete from Modeles where Marque  = {dgvMarque.Rows[position].Cells[0].Value}", GLB.Connection);
                GLB.Cmd.ExecuteNonQuery();
                GLB.Cmd = new SqlCommand($"delete from Voitures where Marque  = {dgvMarque.Rows[position].Cells[0].Value}", GLB.Connection);
                GLB.Cmd.ExecuteNonQuery();
                GLB.Cmd = new SqlCommand($"delete from Marques where id_Marque = {dgvMarque.Rows[position].Cells[0].Value}", GLB.Connection);
                GLB.Cmd.ExecuteNonQuery();
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvMarque.Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < dgvMarque.Columns.Count + 1; i++)
                    {
                        xcelApp.Cells[1, i] = dgvMarque.Columns[i - 1].HeaderText;
                    }

                    for (int i = 0; i < dgvMarque.Rows.Count; i++)
                    {
                        for (int j = 0; j < dgvMarque.Columns.Count; j++)
                        {
                            xcelApp.Cells[i + 2, j + 1] = dgvMarque.Rows[i].Cells[j].Value.ToString();
                        }
                    }
